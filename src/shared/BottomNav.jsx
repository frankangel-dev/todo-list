...
export default function BottomNav() {
    ...
    const {isLoggingOff, handleLogoff} = useLogout()
    ...
    return (
        <nav className={'fixed right-0 bottom-0 left-0 z-50 border-t border-border bg-glass/80 backdrop-blur-md md:hidden'} aria-label={'Mobile navigation'}>
            <div className={'mx-auto flex max-w-lg items-center justify-around px-4 py-2'}>
                ...
                {isAuthenticated &&
                    <button
                        className={'flex min-h-11 min-w-11 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl px-3 py-2 text-error/60 transition-all duration-150 hover:text-error'}
                    ...
                    >
                        ...
                        <span>{isLoggingOff ? 'Logging off...' : 'Log Out'}</span>
                    </button>
                }
            </div>
        </nav>
